...
- WAL은 쓰기만 하는(append-only) 일반 파일이다.
  - MySQL의 복구 로그(redo log)나 주키퍼의 트랜젝션 로그 등이 이에 해당한다.
...
- 분산 트랜젝션을 사용한 중복 처리 방지

  ![Distributed Transaction](./images/6-20-distributed-transaction.png)

> Q. 어차피 분산 트랜젝션을 사용해야 한다면, 외부 저장소까지 사용하는것보다는 애초에 업스트림에 대한 소비 응답에 트랜젝션까지의 처리를 묶는 것이 낫지 않나?
...
> ❗ ACID 속성
> - 원자성(Atomicity): 트랜젝션이 완료되지 않은 상태로 남아있지 않다.
> - 일관성(Consistency): 트랜젝션이 완료된 후 데이터베이스는 일관된 상태를 유지한다.
> - 격리성(Isolation): 트랜젝션이 독립적으로 수행된다.
> - 지속성(Durability): 트랜젝션이 완료된 후 데이터베이스는 영구적으로 보존된다.
...
- 장단점
  ...
  - 단점
    ...
    - 트랜젝션이 길어지면 락을 유지하는 시간이 길어지므로 성능이 저하될 수 있다.
...
- 이 시스템에서는 MSA를 도입하였다.
  ...
  - 하지만 독립적인 DB를 갖는 서비스간에 트랜젝션을 구현하는 것은 상당히 어렵다.
...
- 전통적 메일 서버 아키텍처
  ...
  - <img src="./images/8-3-traditional-mail-server.png" alt="8-3" width="600"/>
...
- 개략적 설계안
  - <img src="./images/8-5-blueprint.png" alt="8-5-blueprint" width="600"/>
...
- 이메일 전송 절차
  - <img src="./images/8-6-email-sending-process.png" alt="8-6-email-sending-process" width="600"/>
...
- 이메일 수신 절차
  - <img src="./images/8-7-email-receiving-process.png" alt="8-7-email-receiving-process" width="600"/>
...
- <img src="./images/8-10-elasticsearch.png" alt="8-10-elasticsearch" width="600"/>
...
<img src="./images/9-1-type-of-storages.png" alt="9-1-type-of-storages" width="600"/>

<img src="./images/9-1-traits-of-storages.png" alt="9-1-traits-of-storages" width="600"/>
...
<img src="./images/9-4-blueprint.png" alt="9-4-blueprint" width="600"/>
...
<img src="./images/9-5-object-upload.png" alt="9-5-object-upload" width="600"/>
...
<img src="./images/9-6-object-download.png" alt="9-6-object-download" width="600"/>
...
<img src="./images/9-8-data-storage-componenet.png" alt="9-8-data-storage-componenet" width="600"/>
...
- **배치 서비스**
  ...
  - 내부적으로 가상 클러스터 지도를 유지하며, 이를 바탕으로 원본과 사본이 다른 노드에 저장되도록 한다.
    - <img src="./images/9-9-virtual-cluster-map.png" alt="9-9-virtual-cluster-map" width="600"/>
...
<img src="./images/9-10-persistant-data-storage-flow.png" alt="9-10-persistant-data-storage-flow" width="600"/>

1. API 서비스가 객체 데이터를 데이터 저장소로 포워딩
2. 데이터 라우팅 서비스는 객체에 UUID를 할당하고, 배치서비스를 통해 데이터를 보관할 노드를 결정
3. 데이터 라우팅 서비스가 UUID와 데이터를 주 데이터 노드에게 전달
4. 주 데이터 노드는 수신한 데이터를 저장하고, 두 개의 부 데이터 노드에 다중화. 완료 후 데이터 라우팅 서비스에 알림
5. 객체의 UUID를 API 서비스에 반환

<img src="./images/9-11-data-consistency-vs-latency-tradeoff.png" alt="9-11-data-consistency-vs-latency-tradeoff" width="600"/>
...
<img src="./images/9-13-data-storage-flow.png" alt="9-13-data-storage-flow" width="600"/>

1. API 서비스는 데이터를 데이터 노드 서비스로 전달
2. 데이터 노드 서비스는 데이터를 파일의 뒤쪽에 append
3. 해당 객체에 대한 새로운 레코드를 관계형 DB 내에 row로 추가
4. 데이터 노드 서비스는 API 서비스에 객체의 UUID를 반환
...
- 일반적으로 하드 디스크의 연간 장애율은 0.81%라고 한다. - [출처](https://www.backblaze.com/blog/cloud-storage-durability/)
...
- 소거 코드의 원리
  ...
  - <img src="./images/9-15-data-recovery-using-erasure-coding.png" alt="9-15-data-recovery-using-erasure-coding" width="600"/>

    1. 데이터를 네 개의 단위 크기로 분할한다.
    2. 수학 공식을 사용하여 패리티 블록(`p1`, `p2`)를 계산한다.
    3. 데이터 `d3`, `d4`가 소실되었다고 가정하면,
    4. 남은 값 `d1`, `d2`, `p1`, `p2`를 사용하여 데이터 `d3`, `d4`를 복구 할 수 있다.
...
- 따라서 이는 샤딩(sharding)을 통해 분할하여 저장함.
  - `bucket_id`를 기준으로 샤딩한다면, 헤비 유저의 버킷이 핫스팟이 되므로 부하가 분산되지 않음.
  - `object_id`를 기준으로 샤딩한다면, 핫스팟은 줄일 수 있지만 앞서 이야기한 1, 2번 질의를 효과적으로 처리하기 어렵다. 이는 두 질의는 URI를 기준으로 처리되기 때문임.
  - 결과적으로 여기에서는 `bucket_id`와 `object_name`을 결합하여 샤딩에 활용할 수 있음.

> Q. URI를 기준으로 처리하는것과 `object_id`를 기준으로 샤딩하는것이 무슨 관계가 있지?
>
> 두 질의는 모두 URI를 기준으로 처리되기 때문에, 객체의 이름이 문자열로 제공됨.
> 따라서 이 문자열을 `object_id`(UUID)로 변환하기 위해 메타데이터가 필요한데, 이는 결국 모든 샤드를 탐색해야 알 수 있음.
> 반면 `bucket_id`와 `object_name`을 결합하여 샤딩하면, 이는 두 문자열을 결합하여 하나의 샤드를 특정할 수 있으므로 효율적임.
...
- 이 작업에서 `abc`이하의 디렉토리 처리는 어플리케이션에서 처리한다.
...
<img src="./images/9-22-object-versioning.png" alt="9-22-object-versioning" width="600"/>

1. 클라이언트가 HTTP로 PUT 요청을 보낸다.
2. API 서비스는 IAM 서비스로부터 사용자의 신원과 권한을 확인한다.
3. 확인결과 이상이 없다면 데이터를 저장소로 업로드한다. 데이터 저장소는 이를 각각의 노드에 복제한 후 UUID를 반환한다.
4. API 서비스는 획득한 UUID를 바탕으로 메타데이터 저장소를 호출하여 메타데이터를 생성한다. 이 때 동일한 이름을 가진 기존 레코드가 있는지, 그리고 버저닝 기능이 활성화 되어있는지를 확인한다.
5. 4번의 두 조건이 만족된다면 새로운 메타데이터를 생성한다. 이때 `bucket_id`, `object_name`은 같지만, `object_id`와 `object_version(TIMEUUID)`이 다른 새로운 레코드를 생성한다.
...
<img src="./images/9-25-multipart-upload.png" alt="9-25-multipart-upload" width="500"/>

1. 클라이언트가 대용량 파일을 업로드 요청을 보낸다.
2. 데이터 저장소가 uploadID를 발급하고, 이를 클라이언트에 반환한다.
3. 클라이언트가 파일을 작은 객체로 분할하여 업로드한다. 여기서는 1.6GB 파일을 8개로 분할하여 각 200MB씩 업로드한다고 가정하며, 각각의 파일을 2에서 얻은 uploadID를 포함하여 업로드한다.
4. 조각 하나를 업로드 할 때 마다 데이터 저장소는 ETag를 클라이언트에 반환한다.
5. 모든 조각이 업로드되면 클라이언트는 완료 요청을 보내며, 이때 uploadID와 각 조각의 ETag를 포함한다.
6. 데이터 저장소는 이를 받아 각 조각을 복원하고 원본 파일을 재구성한다.
...
- 신용카드 결제만 처리
- 전문 결제 서비스업체 사용
- 신용카드 데이터 처리는 결제 서비스 업체에 의존
- 하나의 통화만 사용한다고 가정
- 하루 100만건의 거래 발생
- 매월 판매자에게 정산대금을 지급
- 조정 작업을 통해 불일치하는 부분을 교정

#### 기능 요구사항

- 대금 수신(pay-in) 흐름과 대금 정산(pay-out) 흐름을 구현해야 한다.

#### 비기능 요구사항

- 신뢰성과 내결함성
- 내부 서비스(결제, 회계 시스템)와 외부 서비스(결제 서비스 제공업체)간의 조정 프로세스 필요
  - 시스템 결제 정보가 일치하는지 비동기적으로 확인

#### 개략적인 규모 추정

- 하루 100만건의 거래 발생
  - 편의상 하루를 약 10만 초로 가정하였으므로, 초당 10건의 트랜젝션 발생
- 이는 크지 않은 부하이므로, 일반적인 DB를 사용하여 트랜젝션 처리에 보다 집중하여 설계해도 무방

### 2단계: 개략적 설계안 제시 및 동의 구하기

<img src="./images/11-1-simple-pay-in-out-flow.png" alt="11-1-simple-pay-in-out-flow" width="600"/>

- 결제 시스템에서 자금의 흐름은 크게 두 가지로 나뉜다.
  - 들어오는 대금: 대금 수신(pay-in)
  - 나가는 대금: 대금 정산(pay-out)
...
- 결제 서비스는 사용자로부터 결제 이벤트를 수신하고 프로세스를 조율한다.
  - 가장 먼저 위험 점검(risk check)을 수행한다.
  - 이 과정은 일반적으로 매우 복잡하고 고도화 되어있으므로 결제 서비스 공급자(PSP; Payment Service Provider)에게 위임한다.

##### 결제 실행자

- 결제 실행자는 PSP를 통해 결제 주문(payment order)를 실행한다.
  - 하나의 결제 이벤트에는 여러 결제 주문이 포함될 수 있다.

> Q. 결제 이벤트에 여러 결제 주문이 포함되는 경우가 언제 있을 수 있을까?
>
> 묶음 주문, 혹은 자동 결제 처리 등의 경우에 가능함.
...
- 복식부기(double-entry accounting)이란, 모든 거래를 양쪽의 계좌에 기록하는 회계 시스템이다.
- 이는 모든 거래를 두 개의 계좌에 기록하므로, 이를 통해 모든 거래를 추적하고 검증할 수 있다.

#### 외부 결제 페이지

- 신용카드 정보를 직접 저장하기 위해서는 미국의 PCI DSS(Payment Card Industry Data Security Standard) 규정과 같은 국가별 보안 규정을 준수해야 한다.
  - 이는 복잡한 규정이며, 비용이 많이 들고, 비즈니스 프로세스에 영향을 줄 수 있다.
  - 따라서 일반적인 업체들은 신용카드 정보를 직접 처리하지 않고, 외부 PSP의 결제 페이지를 통해 결제를 처리한다.

#### 대금 정산 흐름

- 대금 정산에는 일반적으로 PSP가 아닌, 별도의 정산 서비스를 사용한다.
  - 정산에도 다양한 규정이 있고, 복잡한 프로세스가 필요하기 때문이다.

### 3단계: 상세 설계

#### PSP 연동

- 회사의 규모가 크고, 은행이나 비자/마스터카드 등과 같이 복잡한 카드 시스템에 직접 연결 할 수 있다면 PSP 없이도 결제 할 수 있다.
- 그러나 이는 많은 비용이 들고, 복잡한 프로세스가 필요하다.
- 따라서 일반적으로 PSP를 사용한다.

<img src="./images/11-4-psp-flow.png" alt="11-4-psp-flow" width="600"/>

1. 사용자가 결제 버튼을 클릭하면 클라이언트가 결제 서비스를 호출한다.
2. 결제 서비슨느 PSP에 '결제 등록 요청(payment registration request)'를 전송한다.
   1. 결제 금액, 통화, 만료일, 리다이렉션URL과 중복 결제 방지를 위한 UUID 등의 정보가 포함된다.
3. PSP는 결제에 사용할 토큰을 결제 시스템에게 반환한다.
   1. 이를 통해 결제를 등록하고 실행상태를 조회 할 수 있다.
4. 결제 서비스는 이 토큰을 DB에 저장한다.
5. 결제 서비스는 클라이언트가 외부 결제 페이지를 표시하도록 한다. 일반적으로 SDK의 형태로 제공된다.
6. 사용자가 PSP의 외부 결제 페이지에 결제 정보를 입력하고 결제를 요청한다.
7. PSP가 이 정보를 토대로 결제상태를 반환한다.
8. 사용자는 리다이렉션 URL로 리다이렉트 된다.
9. PSP는 비동기적으로 결제를 처리하며, 웹훅을 통해 결제 서비스를 호출하여 결제 이벤트를 알린다. 결제 서비스는 이 이벤트를 바탕으로 결제 주문 테이블의 상태를 갱신한다.

#### 조정

- 위와 같은 과정을 비동기적으로 거치기 때문에, 언제든 메시지가 누락될 수 있다.
- 따라서 관련 서비스들의 상태를 주기적으로 점검하여 결과가 일치하는지 확인할 조정 과정이 필요하다.
- 조정 과정에서 확인된 문제점에 대한 처리방안은 크게 아래의 세 경우로 나뉜다.
  1. 어떤 유형의 문제인지 알고, 처리를 자동화 할 수 있는 경우
  2. 어떤 유형의 문제인지 알지만, 처리를 자동화하는 비용이 지나치게 큰 경우
     1. 재무팀에 의한 수동 조정이 필요
  3. 어떤 유형의 문제인지 알지 못하는 경우
     1. 재무팀에 의한 조사와 수동 조정이 필요

#### 결제 지연 처리

- 대부분의 경우 결제 요청은 몇 초 이내에 처리되나, 종종 완료나 거부되기까지 수 시간, 수 일이 걸리는 경우도 있다.
  - PSP가 해당 결제요청의 위험성이 높다고 판단하고 검토하는 경우
  - 신용카드사가 소유자의 추가정보를 요구하는 경우(3D 보안인증; 3D Secure Authentication)
- 이러한 경우 일반적으로 PSP는 웹훅을 통해 결제 서비스에게 상태변경을 알린다. 간혹 결제 서비스가 폴링을 통해 결제 서비스에게 상태를 조회하기도 한다.

#### 내부 서비스 간 커뮤니케이션

- 내부 서비스 간 통신은 크게 동기, 비동기로 나뉜다.
...
- 단점
  - 서비스의 자율성을 높이기 어렵다.
  - 관련서비스 중 하나에 문제가 발생하면 전체 성능에 영향을 끼친다.
  - 특정한 장애를 격리하기가 어렵다.
  - 각 요청의 발신자는 각각의 수신자를 알고있어야 한다.
  - 갑작스러운 트래픽 증가에 따른 확장이 어렵다.

##### 비동기 통신

- 비동기 통신도 두 가지 방식으로 나눌 수 있다.
  - 단일 수신자
    - 하나의 메시지를 하나의 서비스가 소비한다.
  - 다중 수신자
    - 하나의 메시지를 여러 서비스가 각각 소비한다.
    - 예를 들어, 현재 시스템에서 결제, 분석, 청구 서비스 등이 각각 소비하는 경우가 있다.
- 장점
  - 시스템의 자율성을 높이기 쉽다.
  - 확장이 용이하고 장애 감내 능력이 좋다.
- 단점
  - 시스템이 복잡해지고 비용이 많이 든다.
  - 데이터 일관성을 유지하기 어렵다.

#### 결제 실패 처리

<img src="./images/11-10-failed-message-process.png" alt="11-10-failed-message-process" width="600"/>

1. 재시도가 가능한지 확인한다.
2. 재시도 큐에 쌓인 이벤드를 재시도한다.
3. 재시도 임계값을 넘은 큐는 실패 큐에 쌓는다.

#### 정확히 한 번 전달

- 정확히 한 번 전달은 논리적으로 아래 두 조건의 중첩으로 표현 할 수 있다.
  - 최소 한 번은 실행된다.
  - 최대 한 번만 실행된다.

##### 재시도

- 재시도 매커니즘을 통해, 첫번째 조건인 '최소 한 번은 실행된다.'를 보장 할 수 있다.
- 재시도의 전략은 아래와 같은 종류가 있다.
...
>
> 재시도 간격을 지정하는 헤더이다.
> 일반적으로 429(Too Many Requests), 503(Service Unavailable) 등의 응답 코드와 함께 사용되며, 사용방식은 두 가지가 있다.
>
> 1. 초 단위
>
> ```
...
- '최대 한 번만 실행된다.'라는 조건을 보장하기 위한 가장 중요한 개념이다.
- API 관점에서 [멱등성은 이와 같이 정의](https://www.rfc-editor.org/rfc/rfc7231#section-4.2.2)된다.

##### 일관성

- 분산 환경에서는 통신 실패로 인한 데이터 불일치가 발생할 수 있다.
- 이로인해 조정 절차가 꼭 필요하다.

##### 결제 보안

- 결제 보안을 위한 몇가지 기술을 소개한다.
...
| 데이터 손실 | 여러 지역에 걸쳐 DB 복제 및 스냅샷 백업 |
| 분산 서비스 거부 공격(DDOS) | 처리율 제한 및 방화벽 |
| 카드 도난 | 카드 번호 토큰화(tokenization) |
| PCI 규정 준수 | PCI DSS(Payment Card Industry Data Security Standard) 준수 |
| 사기(fraud) | 주소 확인, 카드 확인번호(CVV), 사용자 행동분석 등 |

> ❗인증서 고정(certificate pinning)
>
> 모바일 앱이나 웹 클라이언트가 특정 서버와 통신할 때 신뢰할 수 있는 인증서나 공개키만 허용하는 보안 기술.
>
> ### 기본 원리
> 
> 1. 앱/클라이언트 개발 시 특정 서버의 인증서나 공개키 정보를 미리 내장
> 2. 서버와 연결할 때 제공받은 인증서가 저장된 정보와 일치하는지 확인
> 3. 일치하지 않으면 연결을 거부

### 4단계: 마무리

- 여태 다룬 내용 이외에도, 결제 시스템은 아래와 같이 훨씬 복잡하고 고도화된 기능을 요구한다.
  - 모니터링: 트래픽 패턴, 처리율, 응답시간, 장애 등 주요 지표에 대한 가시성
  - 경보: 장애 발생 시 즉시 알림 및 대응 준비
  - 디버깅 도구: 오류 추적 및 문제 해결
  - 환율: 통화 변환 처리
  - 지역: 지역별 결제 수단의 다양성과 현지화 지원
  - 현금 결제: 현금 결제 처리
  - 구글/애플페이 등 소셜 결제 연동: 사용자 편의를 위한 소셜 결제 기능
