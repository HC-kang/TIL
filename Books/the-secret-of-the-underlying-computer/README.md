...
- 주요 개념
  - CPU, 운영체제, 수치 체계, 스레드, 프로그래밍 언어
  - CISC, RISC
  - 스택
    - 함수 호출, 시스템 호출, 인터럽트 처리, 스레드 전환의 구현방식
...
- CPU는 필요한 데이터가 있으면 먼저 L1, L2, L3 캐시를 순서대로 확인한다.
  - 캐시에 데이터가 있다면, CPU는 캐시에서 데이터를 읽어들인다. 이를 `캐시 히트`라고 한다.
  - 만약 캐시에 데이터가 없다면, 메모리에서 데이터를 새로 읽어들인다.
    - 이 때 캐시에 데이터를 다시 저장하는 것을 `캐시 미스`라고 한다.
...
- 컴파일시 코드가 재정렬되는 예시
  ...
  - 생성 과정에서 최적화를 실행한 어셈블리 코드
    ...
    ```asm
    mov  0x200b54($rip),%eax  # %eax = b
    movl $0xc8,0x200b41(%rip) # b = 200   <--- 이 부분의 순서가 변경됨
    add  $0x64,%eax           # %eax = %eax + 100
    mov  %eax,0x200b4f(%rip)  # a = %eax
    ```
...
- `LOAD`와 `STORE` 명령어를 통해 메모리에 접근할 수 있다.
