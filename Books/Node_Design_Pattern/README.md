...
- Node.js는 분산 시스템을 구축하기에 적합한 도구임.
- 노드의 특성상, 개발자는 다른 언어를 사용 할 때 보다 확장의 필요성을 빠르게 당면함.
  - 멀티코어를 사용 할 수 없는 언어적 특성 때문
- 이러한 확장의 목적에는 성능 이외에도 다른 여러 요소가 있음.
  - 가용성
  - 내결함성 등
- 주요 개념
  - 확장성에 관심을 가져야 할 이유
  - 스케일 큐브
  - 확장성을 이해해야 하는 이유
  - 동일 앱을 여러 인스턴스로 실행하여 확장하는 방법
  - 앱을 확장 할 때 로드밸런서를 활용하는 방법
  - 서비스 레지스트리
  - k8s와 같은 오케스트레이션 플랫폼 사용
  - 모놀리식에서 MSA로 전환하는 방법
  - 간단한 아키텍처 패턴으로 서비스들을 통합하는 방법

### 12-1 애플리케이션 확장 소개

- 확장성: 애플리케이션이 끊임없이 변화하는 조건에 적응하고, 성장하는 능력임.

12-1-1 Node.js 애플리케이션 확장

- Node.js의 특성상 대부분의 작업 부하가 단일 스레드의 컨택스트에서 발생함.
  - 따라서 I/O바인딩 작업에는 유리하지만, CPU 바인딩 작업에는 적합하지 않음.
- 이러한 문제를 해결하는 유일한 방법은, 앱을 여러 프로세스와 시스템에 걸쳐 확장하는 것 뿐임.
- 또한 이를 통해 고가용성과 내결함성 또한 달성 할 수 있음.
...
- 스케일 큐브
  - 축의 개념
    - X축: 복제, 수평적 확장
    - Y축: 분할, 서비스/기능별 분해
    - Z축: 분할, 데이터베이스/테이블별 분해
  - 예시
    - (0, 0, 0): 단일 인스턴스, 단일 데이터베이스 - 모놀리식
    - (1, 1, 1): 여러 인스턴스, 여러 데이터베이스 - MSA
  - 축별 상세
    - X축: 복제, 수평적 확장
      - 애플리케이션을 여러 인스턴스로 복제하는 것
      - 상대적으로 저렴하고, 쉽게 구현 가능
    - Y축: 분할, 서비스/기능별 분해
      - 애플리케이션을 여러 서비스로 분할하는 것
      - 인증, 인벤토리, 결제 등의 서비스로 분할
      - 개발 및 운영 관점에서도 큰 영향을 끼침
      - 마이크로 서비스가 대표적인 예시
    - Z축: 분할, 데이터베이스/테이블별 분해
      - 각 인스턴스가 데이터의 일부만을 처리하는 것
      - 국가, 사용자 등의 기준으로 데이터를 분할
      - 각 데이터 파티션별로 다른 인스턴스가 처리
      - 따라서, 작업 전 인스턴스를 판별하기 위한 조회 단계가 필수적임.

### 12-2 복제 및 로드 밸런싱

- 기존 멀티스레드 환경에서는 서버의 모든 처리 능력을 활용 할 수 있음.
  - 따라서 리소스를 더이상 업그레이드 할 수 없거나, 비용상 한계가 왔을 때 확장이 필요함.
- 그러나 Node.js의 경우 이와 다르게 더 빠르게 확장의 필요성이 생김
...
12-3-2 마이크로 서비스 아키텍처

- Node.js에서 가장 중요한 철학
  - "거대한 애플리케이션을 만들지 마라"
- 그렇다면, 거대한 기능을 만들기 위해서?
  - 서비스 및 기능에 따른 분해 및 분할이 필요함.
- 마이크로 서비스 아키텍처: 애플리케이션을 여러 서비스로 분해하는 아키텍처
  - 데이터의 소유권: 각 서비스가 자신의 데이터를 소유함.
    - 적절한 독립성을 유지하기 위해 필요함.
    - 그렇다고 공유하는 데이터가 없는것은 아님.
      - 이러한 공유하는 데이터는 장차 모듈간의 결합도를 증가시킬 수 있음.
  - 모놀리식에 비해 본질적으로 더 많은 통신이 필요함.
  - 모놀리식에 비해 정보의 은닉 수준이 훨씬 높음.
  - 필요한 모듈만 독립적으로 확장 할 수 있음.
- 모든 서비스는 소모품임
  - 서비스는 언제든지 죽을 수 있고, 새로운 서비스로 대체 될 수 있음.
- 마이크로 서비스의 단점
  - 통합, 배포 및 코드 공유 측면에서 복잡성이 증가함.

12-3-3 마이크로 서비스 아키텍처의 통합 패턴

- MSA에서 가장 어려운 문제는, 모든 노드를 서로 연결하여 공동의 작업을 수행하는 것임.
  - 대부분, 서로가 서로의 최신 정보를 알고있어야 함.
- 또한, 서비스 간 어떤 결합을 도입할지를 고려하는것도 중요함.
  - API 프록시
    - 서비스 간의 직접적인 통신을 허용하지 않고, API 게이트웨이를 통해 간접적으로 통신하는 방법
    - 본질적으로 리버스 프록시의 형태임.
    - 서로 다른 API에 대한 접근을 통합하여 단일 뷰를 제공함.
  - API 오케스트레이션
    - 여러 서비스를 조합하여 단일 API를 제공하는 방법
    - API 게이트웨이를 통해 단일 API를 제공함.
    - 데이터 집계, 데이터 변환 등의 작업을 수행함.
    - 디버그와 확장이 쉽지만, 작동방식에 대한 높은 이해를 필요로 함.
    - 너무 많은것을 알고있는 GOD 객체 패턴을 피해야 함.
      - 이러한 문제를 해결하기 위해 메시지 브로커를 사용함.
      - 메시지 브로커를 통해 pub/sub으로 상호작용의 복잡성을 줄일 수 있음.

### Chapter 12 요약

- 애플리케이션의 확장은 단순히 더 단위시간당 더 많은 트래픽을 처리하기 위해서만 사용되는 것이 아님.
- 더 나은 가용성과 장애 복원력을 제공함.
- 이런 관점에서, Node.js의 약간은 강제되지만 간편한 확장성은 나쁘지 않은 선택임.
- 이러한 확장 중에서도 목적별로 다양한 방법이 있었음
  - 스케일 큐브의 X축: 수평 확장, 인스턴스의 수 늘리기
  - 스케일 큐브의 Y축: 기능 분해, 서비스 분리

## Chapter 13. 메시징과 통합 패턴

- 이전 챕터에서는 확장에 대해 다루었고, 이번 챕터에서는 통합에 대해서 다룰것임.
- 여러 프로세스와 인스턴스로 분할된 서비스를 원활하게 운영하려면, 이들간의 통신이 필수적임.
- 분산된 서비스의 대표적인 통합 방법
  - 공유 스토리지를 중재자(coordinator)와 정보 관리자로 사용
  - 메시지를 사용하여 서비스간 이벤트와 명령을 공유하는 것
    - 메시징 시스템: 네트워크를 통한 정보공유를 용이하게 하는 솔루션, 패턴 및 아키텍처
    - 메시지: 특정 주제에 대한 정보를 전달하는 데이터의 단위
  - 메시징 시스템의 종류
    - 브로커 방식
    - 피어 투 피어 방식
    - 요청/응답 메시지 교환
