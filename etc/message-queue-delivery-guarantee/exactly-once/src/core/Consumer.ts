...
export class Consumer {
  ...
  async processMessage(
    ...
    // Redis(외부 저장소)에서 처리여부 확인
    const isProcessed = await this.adapter.isMessageProcessed(message.id);
    if (isProcessed) {
      console.log(
        `[${this.name}] Duplicate message detected: ${message.id} - Acknowledging without processing`
      );
      // 중복 메시지는 처리하지 않고 바로 ACK 처리
    ...
    try {
      ...
      // 메시지 처리 성공 시 Redis에 저장 (분산 환경에서 중복 처리 방지)
  ...
  private async retryPendingMessages(): Promise<void> {
    ...
    for (const message of pendingMessages) {
      ...
      if (isProcessed) {
        console.log(
          `[${this.name}] Duplicate pending message detected: ${message.id} - Acknowledging without processing`
        );
        // 이미 처리된 메시지는 처리하지 않고 바로 ACK 처리
        await this.adapter.acknowledgeMessage(message);
        continue;
      }
