...
async function runMessageQueueSystem(
  ...
  const queueName = 'test-queue-exactly-once';
  ...
  // 컨슈머 설정 - Redis를 통해 처리된 메시지 ID 공유
...
function compareMessageLists(
  ...
  // 4. 중복 시도되었지만 처리되지 않은 메시지 분석 (exactly-once 특성)
  ...
  if (duplicateMessages.length === 0) {
    ...
    console.log(`  총 ${duplicateMessages.length}개 메시지가 중복 시도됨:`);
  ...
  const isAllProcessedOnce = uniqueProcessedCount === totalExpectedCount;

  console.log('\n[4] 전체 처리 통계:');
  console.log(`  - 생산된 메시지 수: ${totalExpectedCount}개`);
  console.log(`  - 처리된 고유 메시지 수: ${uniqueProcessedCount}개`);
  console.log(`  - 중복 시도되었지만 처리되지 않은 메시지 수: ${duplicateMessages.length}개`);

  console.log('\n[5] 최종 검증 결과:');
  if (isAllProcessedOnce) {
    console.log('  ✅ 성공: 모든 메시지가 정확히 1회 처리되었습니다.');
  } else {
    console.log(
      `  ❌ 실패: ${
