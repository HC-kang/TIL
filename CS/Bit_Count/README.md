...
예를 들어, `1010`, `0110`, `0010`, `0001`의 경우 각각 4비트 내에서 1의 개수는 2, 2, 1, 1입니다. 그리고 이들을 각각 상위, 하위 비트로 나누어 `0x33333333`과 마스킹하면 각각 상위는 `10`, `01`, `00`, `00`, 하위는 `10`, `10`, `10`, `01`이 됩니다. 

눈썰미가 좋은 분들은 이미 캐치하셨을텐데, 여기서 중요한점은 이전 계산 결과에 따라 각각의 상하위 비트는 `['10', '01', '01', '00']`밖에 가질 수 없다는 점입니다.(2비트에서 1의 개수는 2보다 작거나 같아야 하기 때문)

위의 예시 숫자들은 10진수로 각각 2, 1, 0, 1과 2, 2, 2, 1이 되고 이를 더하면 각각 4, 3, 2, 2가 됩니다. 이 단계의 목표인 4비트 2진수로는 `0100`, `0011`, `0010`, `0010`이 되는 것입니다.

`1010`, `0110`, `0010`, `0001`에 대해 결론을 10진법으로 예로 들어보자면, 두 개씩의 숫자로 이루어진 `22`, `12`, `02`, `01`을 각각 `0100`, `0011`, `0010`, `0010`, 즉 `4`, `3`, `2`, `1`로 압축하 것입니다.

| 4비트 2진수 | 2비트 2진수 | 10진수 |
|:---:|:---:|:---:|
| 0100 | 10 10 | 4 |
| 0011 | 01 10 | 3 |
| 0010 | 00 10 | 2 |
...
이렇게 계산하면 `i = 0x20`이 됩니다. 그리고 이는 10진수로 `32`가 됩니다. 이렇게 `Integer.bitCount` 함수가 동작하는 방식을 한번 직접 보았습니다.