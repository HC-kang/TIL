...
스터디 첫 주차에는 [191. Number of 1 Bits](https://leetcode.com/problems/number-of-1-bits) 문제를 풀었습니다. `easy` 등급 문제라 어렵지는 않았지만, `bitCount` 함수를 발견한 후 그 원리를 이해해보고 싶었습니다.
...
먼저, 이 함수는 비트연산자를 사용하고 있습니다. 비트연산자는 이미 잘 알려져 있긴 하지만, 간단히 설명드리겠습니다.
...
우선은 만만한 숫자를 뽑아 `i = 42`로 예를 들어보겠습니다.  
...
피상적으로는 알겠는데, 이게 무슨 의미일까요? 우선은 잘 모르겠으니, 한번 직접 계산해 보겠습니다.
...
자 이렇게 계산하면 결과는 `i = 21`이 됩니다.  
이렇게 2비트씩 묶어서 1로 만드는 이유는 무엇일까요? 아직은 잘 모르겠고 와닿지 않습니다. 좀 더 알아보겠습니다.
...
이번에는 마스킹을 두 번이나 하고 있습니다. 이게 무슨 의미일까요? 우선은 계속 계산해보겠습니다.
...
이렇게 계산하면 `i = 18`이 됩니다. 이렇게 4비트씩 묶어서 1로 만드는 이유는 무엇일까요?  
글쎼요.. 아직도 잘 와닿지가 않는 것 같습니다... 무슨 근거로 이렇게 계산하는걸까요?

### 3단계 - 8비트씩 묶어서 1로 만들기

> `i = (i + (i >>> 4)) & 0x0f0f0f0f;`

이번에는 `i`와 `i`를 오른쪽으로 4비트씩 시프트한 값을 더하고, `0x0f0f0f0f`와 `&` 연산한 값을 `i`에 대입하고 있습니다.  
이게 뭘 의미하는지는 점점 모르겠네요.  
게다가 방금 전에는 두 번 진행한 마스킹을 이번에는 한 번만 적용했네요. 그렇지만 여전히, 마스크를 보면 뭔가 분명히 의미는 있는것 같죠?
...
자, 이렇게 계산하면 `i = 3`이 됩니다.  
뭔가 진행이 되고는 있는 것 같은데, 아직까지도 갈피를 못잡겠습니다.  
이렇게 8비트씩 묶어서 1로 만드는 이유는 무엇일까요? 전개되는 모양을 보면 뭔가 규칙이 있을 것 같은데, 아직까지도 잘 모르겠습니다.
...
이번에는 심지어 마스킹을 하지 않았습니다. 대체 무슨 규칙이 있는걸까요?
...
다만 여기서 `0x3f`는 2진수로 표현하면 `0000 0000 0000 0000 0000 0000 0011 1111`이고, 마지막 6비트만 남기는 것을 볼 수 있습니다.  
...
오.. 아직 정확히는 모르겠지만 결과적으로 정답인 `3`이 나와있네요.  
아직 정확히는 모르겠지만 `Integer.bitCount` 함수가 어찌 동작은 한다는 결과를 얻었습니다.
...
좀 더 확실한 이해를 위해, `i = 42`가 아닌, 32비트의 최대값인 `i = 0xffffffff`를 가지고 계산해보겠습니다.
...
각각의 2비트를 `11`, `10`, `01`, `00`로 예를 들어보겠습니다. 이 경우 각각 2비트 내에서 1의 개수는 2, 1, 1, 0개 입니다.  
...
| 2비트 | 2비트(10진수) | 상위 1비트 | 상위 1비트(10진수) | 차이 | 새로운 2비트 |
|:---:| :---:| :---: | :---: | :---: | :---: |
| 11 | 3 | 1 | 1 | 2 | 10 |
| 10 | 2 | 1 | 1 | 1 | 01 |
| 01 | 1 | 0 | 0 | 1 | 01 |
...
즉, `1111 1111 1111 1111 1111 1111 1111 1111`가 `1010 1010 1010 1010 1010 1010 1010 1010`으로 변하게 됩니다.  
...
위의 결과처럼 앞, 뒤 16비트의 결과가 `0000 1000 0001 0000`로 나오는데, 이는 10진수로 2064이 나오게 됩니다. 이전까지의 결과대로라면 `0000 0000 0001 0000`로 16이 나와야 하는데 말이죠.  
...
이렇게 계산하면 `i = 0000 0000 0000 0000 0000 0000 0010 0000 = 0x 0000 0020`이 됩니다. 그리고 이는 10진수로 `32`가 되죠.  
