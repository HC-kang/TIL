...
여기서도 위와 비슷하지만, 이번에는 4비트 내에서 1의 개수를 세기 위함입니다.
`0x33333333`과 마스킹하면 각각의 4비트 내에서 하위 2비트의 값이 얻어집니다.
그리고 오른쪽으로 2비트씩 시프트한 수를 `0x33333333`로 마스킹하면 각각의 4비트 내에서 상위 2비트의 값이 얻어집니다.

예를 들어, `0011`, `0010`, `0010`, `0001`의 경우 각각 4비트 내에서 1의 개수는 2, 2, 1, 1입니다. 그리고 이들을 각각 상위, 하위 비트로 나누어 `0x33333333`과 마스킹하면 각각 상위는  '''이 됩니다.

여기서 중요한점은, 이전 계산 결과에 따라 각각의 상하위 비트는 `['10', '01', '01', '00']`밖에 가질 수 없다는 점입니다(2비트에서 1의 개수는 2보다 작거나 같아야 하기 때문)
