{
  ...
  class PatternPrinterBuilder {
    private patternType: string | null = null;
    private starFactory: StarFactory;
    private spaceFactory: SpaceFactory;
    private observers: Observer[] = [];
    ...
    setPatternType(patternType: string): PatternPrinterBuilder {
    ...
    build(): PatternPrinter {
      if (this.patternType === null) {
        throw new Error('Pattern type must be set before building');
      }
      let pattern: StarPattern;

      switch (this.patternType) {
        case 'right':
          pattern = new RightAlignedStarPattern(
            this.starFactory,
            this.spaceFactory
          );
          break;
        case 'left':
          pattern = new LeftAlignedStarPattern(
            this.starFactory,
            this.spaceFactory
          );
          break;
        case 'equilateral':
          pattern = new EquilateralStarPattern(
            this.starFactory,
            this.spaceFactory
          );
          break;
